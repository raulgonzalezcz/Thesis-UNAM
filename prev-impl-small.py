...
def set_weights(graph):
	...
	for e in graph.edges:
		#graph.add_edge(e[0], e[1], weight=round(random.uniform(-1,1),4))

		w = round(random.choice(possible_states))
		m_weights[e[0]][e[1]] = w
		m_weights[e[1]][e[0]] = w
	return m_weights
...
# Synchronus update
def update_weights(number_nodes,m_weights,states,delta):
	for i in range(number_nodes):
		for j in range(number_nodes):
			if(i!=j):
...
# Plot energy frequency results
def plotResultsMoveMethod(max_updates, dataNL, dataWL, title):
	print("Plotting results")
	x = [i for i in range(max_updates)]
	#print(x)
	#print(y)
...
def run_simulation(graph,m_weights,tests):
	...
	return dict_states, tests_record,atractors_record
	...
	for gen in range(number_tests):
		...
		for i in range(relaxation_length):
			...
			if counter_atractor > times_atractor:
				...
				m_weights_l = update_weights(N,m_weights_l,dict_states,delta)
...
tests = 5
number_nodes = 100
most_connections = math.floor(number_nodes / 10)
graph = nx.random_graphs.watts_strogatz_graph(number_nodes,most_connections,0.5)  #Nodes, most connections, probability of conection
...
states, tests_recordNL, bestNL = run_simulation(graph,m_weights,tests)
# Show results
#plot_graph(graph, states)

# Self-modeling with learning
print("\nStarting simulation with learning...")
states, tests_recordWL, bestWL = run_simulation_with_learning(graph,m_weights,tests)
# Show results
#plot_graph(graph, states)

# Show comparison
plotResultsMoveMethod(10*number_nodes,tests_recordNL, tests_recordWL, "Atractors states visited without and with learning")
plotResultsFrequencyMethod(10*number_nodes,bestNL, bestWL, "Frequency of atractors without and with learning")
