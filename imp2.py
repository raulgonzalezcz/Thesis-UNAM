""" 
Unsupervised learning applied to complex networks.
...
def set_states(graph):
    '''Set initial states of the network.'''
    # States: {1,-1}
    possible_states = [-1,1]
    for value in range(nx.number_of_nodes(graph)):
        # Add/ modify state as a key value
        graph.nodes[value]["state"] = random.choice(possible_states)
 
# Asynchronus update
def update_states(graph):
    # Pick one neuron randomly
    unit = random.randint(0,nx.number_of_nodes(graph)-1)
    pound = 0
...
def get_energy(graph):
    ...
    for i in range(0,nx.number_of_nodes(graph)-1):
        for j in list(graph.adj[i]):
            energy += graph[i][j]["weight"] * graph.nodes[i]["state"] * graph.nodes[j]["state"]
            #print(u,v,w)
    energy *= -5
    #print(energy)
    return energy
...
# Plot energy results
def plotResultsMethod(name_folder,max_updates, dataB, title):
    ...
    plt.savefig(name_folder+title+'.png', dpi=100)
    plt.clf()
 
# Plot energy frequency results
def plotResultsFrequencyMethod(name_folder,max_updates, dataNL, dataWL, title):
    width = 0.2
...
# Plot energy frequency results
def plotResultsMoveMethod(name_folder,bestBL, bestDL, bestAL, title):
...
def run_simulation(name_folder,graph,tests,relaxation_length,label_plot):
    atractors_record = []   
    tests_record = []
    ...
    plotResultsMethod(name_folder,relaxation_length,tests_record, label_plot)
    return atractors_record
 
def run_simulation_with_learning(name_folder,graph,tests,relaxation_length,times_atractor,label_plot):
    delta = 0.0001/nx.number_of_nodes(graph)
...
# Configuration variables for network
tests = 1000
number_nodes = 35
...
name_folder = './Test/'+str(milli_sec)+'/'
createFolder(name_folder)
 
# Configuration for learning 
relaxation_length = 30*number_nodes
#times_atractor = int( math.pow(3/4,N)*math.log(N) )
times_atractor = int( 3/4 *math.exp(1)*number_nodes*math.log(number_nodes) )
print("Atractor pattern selected", times_atractor)
...
bestBL = run_simulation(name_folder,graph,tests,relaxation_length,'Self-modeling of network before learning')
get_edge_data(graph)
# Show results
#plot_graph(graph)
...
bestDL = run_simulation_with_learning(name_folder,graph,tests,relaxation_length,times_atractor,'Self-modeling of network during learning')
get_edge_data(graph)
# Show results
# plot_graph(graph)
...
bestAL = run_simulation(name_folder,graph,tests,relaxation_length,'Self-modeling of network after learning')
get_edge_data(graph)
 
# Show comparison
plotResultsMoveMethod(name_folder,bestBL, bestDL, bestAL, "Atractors states visited before, during and after learning")
plotResultsFrequencyMethod(name_folder,10*number_nodes, bestBL, bestAL, "Frequency of atractors without and with learning")
 
print('Folder: '+ str(milli_sec))
